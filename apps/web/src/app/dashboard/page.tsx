...
const demoData: DashboardData = {
    ...
    weeklyTrend: [
        { date: '2026-04-08', value: 7234 },
        { date: '2026-04-09', value: 9123 },
        { date: '2026-04-10', value: 8456 },
        { date: '2026-04-11', value: 10234 },
        { date: '2026-04-12', value: 6789 },
        { date: '2026-04-13', value: 11234 },
        { date: '2026-04-14', value: 8432 },
    ],
    monthlyTrend: [
        { date: '2026-W13', value: 8234 },
        { date: '2026-W14', value: 8567 },
        { date: '2026-W15', value: 9123 },
        { date: '2026-W16', value: 8890 },
    ],
    goals: [
        {
            id: '1',
...
async function loadDashboardFromApi(): Promise<DashboardData | null> {
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        const loadDashboard = async () => {
            try {
                const apiData = await loadDashboardFromApi();
                if (!cancelled) {
                    setData(apiData ?? demoData);
                    setUsingDemoData(!apiData);
                }
            } catch {
                if (!cancelled) {
                    setData(demoData);
                    ...
                    <div className="bg-slate-800/50 backdrop-blur rounded-2xl p-6 border border-slate-700/50">
                        <div className="flex items-end gap-2 h-40">
                            {data.weeklyTrend.map((day) => {
                                const maxValue = Math.max(...data.weeklyTrend.map((item) => item.value));
    ...
    return (
        <div className="bg-slate-800/50 backdrop-blur rounded-2xl p-5 border border-slate-700/50">
            <div className="flex justify-between items-start mb-4">
                <span className="text-sm font-medium text-slate-300">{goalLabel}</span>
